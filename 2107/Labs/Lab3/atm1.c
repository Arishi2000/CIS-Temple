...
// this functions validates PIN, at every time a transaction is being done
int PIN() {
   int i, PIN;
  
   // ask 3 times until, a correct PIN is entered.
   for(i=0; i<3; i++) {
...
// this function validates withdrawals.
int Withdrawl(int rec, int with, int bal) {
    int i, amount = 0;
  
   // ask three times, until valid amount is entered.
   for(i=0; i<3; i++) {
   ...
   if(bal < amount) {
       printf("\nYou don't have enough balance to withdraw $%d\n", amount);
       return 0;
   }
   with += amount;
   if(with<=1000) {
       if(rec==1) printf("\nSuccessfully withdrawn $%d. Please collect the withdrwal reciept\n", amount);
       if(rec==0) printf("\n--- Withdrawal success !! -----\n");
   }
...
// this function validates deposits.
int Deposit(int rec, int dep) {
   int i, amount = 0;
  
   // ask 3 times, until valid amount is entered.
   for(i=0; i<3; i++) {
   ...
   dep += amount;
   if(dep<=10000) {
       if(rec==1) printf("\nSuccessfully deposited $%d. Please collect deposit reciept\n", amount);
       if(rec==0) printf("\n--- Deposition success !! -----\n");
   }
   return amount;
           ...
           case 3 : // deposits part
               ...
               if((amount = Deposit(reciept, deposited))) {
                       ...
                       if(deposited<=10000) { // deposit should not be more than 10000
                           ...
                           printf("\n It is not allowed to deposit more than $10000 in one day\n");
