...
public class MaxPQ<Key> {
   ...
   {
      // loop to check if the k is greater than 1 and using the helper method less
      while ( k > 1 && less( k / 2, k ) )
      {
         exch( k, k / 2 ); // use the exch method and divid it by 2
         k = k / 2; // update k and divid it by 2
      }
   }
   // sink method
   private void sink( int k )
   {
      // loop as long as k multiplied by 2 is less or equal to n keep looping
      while ( 2 * k <= n )
      {
         int j = 2 * k;
         if ( j < n && less( j, j + 1 ) ) j++;
